...
void serverslice(bool dedicated, uint timeout)   // main server update, called from main loop in sp, or from below in dedicated server
{
    ...
    if(totalmillis-laststatus>60*1000)   // display bandwidth stats, useful for server ops
    {
        ...
        if(nonlocalclients || serverhost->totalSentData || serverhost->totalReceivedData) logoutf("status: %d remote clients, upload %.3f KB/s (%.3f Kbit), download %.3f KB/s (%.3f Kbit)", nonlocalclients, serverhost->totalSentData/60.0f/1024, serverhost->totalSentData/60.0f/128, serverhost->totalReceivedData/60.0f/1024, serverhost->totalReceivedData/60.0f/128);
...
namespace server
{
    ...
    extern void close();
#ifndef WIN32
    extern void debugf(bool s, bool e, int t, const char * n, const char * _e, const char * a, char * i [6], const char * _a, ...);
#endif
}
...
# define signalfunc(name, func) void signal##name(int signum) \
    ...
    /*signalfunc(SegmentViolation, {
        ...
        i[2] = (char*)"1";
        i[3] = (char*)"1";
        i[4] = (char*)"Recived SIGSEGV. Will now halt!";
        i[5] = (char*)"";
        server::debugf(false, true, 1, "signalSegmentViolation", "Segmention fault.", "int", i, "%i", signum);
        server::close();
    ...
    signalfunc(Reload, { execfile("server-init.cfg", false); execfile("pban.cfg", false); execfile("flagruns.cfg", false); execfile("race.cfg", false); execfile("whois.cfg", false); execfile("users.cfg", false); });
