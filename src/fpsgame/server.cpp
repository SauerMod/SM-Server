...
/*            case SOUNDHACK:
    ...
    ICOMMAND(isbestrace, "", (), {
        loopv(raceruns)
        {
            if(!strcmp((const char *)raceruns[i].map, smapname))
            {
                if(raceruns[i].millis > ((uint)gamemillis - 5000)) intret(1);
                else intret(0);
                return;
            }
        }
    ...
    ICOMMAND(addbestrace, "s", (const char *runner), {
        loopv(raceruns)
        {
            ...
            {
                if(raceruns[i].millis > ((uint)gamemillis - 5000)) raceruns.remove(i);
                else return;
            }
        }
        ...
        cur.millis = ((uint)gamemillis - 5000);
    })
...
# endif
    ...
    vector<client_whois> clients_whois;

    void add_client_whois(const char *address, const char *name)
    {
        client_whois *cur = 0;
        loopv(clients_whois) if(!strcmp(clients_whois[i].addr, address))
        {
            cur = &clients_whois[i];
            bool needs_add = true;
            for(int j = 0; clients_whois[i].names[j][0]; j++) if(!strcmp(clients_whois[i].names[j], name)) needs_add = false;
            if(needs_add)
            {
                char names[5][260];
    ...
    ICOMMAND(addwhois, "ss", (const char *a, const char *n), {
        add_client_whois(a, n);
    })

    void whois(clientinfo *ci, clientinfo *cx) // ci = requester, cx = requested
    {
        if(!ci || !cx) return;
        const char *address = getclienthostname(cx->clientnum);
        loopv(clients_whois)
        {
            if(!strcmp(address, clients_whois[i].addr))
            {
                string msg;
    ...
    servcmd(whois, PRIV_AUTH, "<cn[,cn2[,...]]>", "displays a client's last 5 used names.", {
        char *array[2];
        explodeString(args, array, ' ', 2);
        if(!array[0]) { sendmsg(ci, "Usage: #whois <cn[,cn2[,...]]>"); return; }
        char *cns[128];
        explodeString(array[0], cns, ',', 128);
        for(int i = 0; cns[i]; i++)
        {
            clientinfo *cx = getinfo(atoi(cns[i]));
            if(!cx) { sendmsgf(ci, "Unknown client num: %i", atoi(cns[i])); continue; }
    ...
    void close()
    {
        ...
        if(fff)
        {
            fff->printf("// Automatically generated by SM-Server - do not edit.\n");
        ...
        if(ffff)
        {
            ...
            ffff->printf("// Contains a list of names used by various cilents.\n");
            loopv(clients_whois)
            {
